  ...
  try {
    if (Utils.fileExistsSyncCache(path.resolve(process.cwd(), argv.name))) {
      if (argv.force) {
        ...
        Utils.warn(
          `Existed ${argv.name} is deleted before creating a new one!`
        );
      } else if (!argv.merge) {
          ...
          const answers: any = await Utils.inquirer.prompt([
            {
              ...
              choices: Object.keys(repos).map((key) => {
                return {
                  name: `${Utils.chalk.green.underline(
                    repos[key].name
                  )} ${repos[key].tags
                    .map((tag) => Utils.chalk.white.bgGreen(` ${tag} `))
                    .join(" ")}: ${Utils.chalk.white(repos[key].repo)}${
