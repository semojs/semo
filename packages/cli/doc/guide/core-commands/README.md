...
╟──────────────────────┼─────────────────────┼────────────────────────────────────────────────────╢
║ hook_new_repo        │ semo              │ Hook triggered in new command.                     ║
╟──────────────────────┼─────────────────────┼────────────────────────────────────────────────────╢
```
...
```
semo new semo-plugin-xxx --select=plugin
```
...
```bash
semo make command make/test --extend=semo
```
...
```
semo new semo-plugin-[PLUGIN_NAME] -yfie
```
...
```
semo new PROJECT_NAME --select=demo
semo new PROJECT_NAME --select=demo_repo
```
...
当然可以手动建脚本，然后用这个命令来触发，但是因脚本还需要起名字，而且还有一定的格式要求，所以，推荐使用 `semo make script` 命令来生成。

```
semo make script test
```
...
[semo-plugin-x]
  version  :  0.0.13
  location :  ~/.nvm/versions/node/[VERSION]/lib/node_modules/semo-plugin-x

[semo-plugin-y]
  version  :  1.8.39
  location :  ~/.nvm/versions/node/[VERSION]/lib/node_modules/semo-plugin-y

[semo-plugin-z]
  version  :  0.0.2
  location :  ~/.nvm/versions/node/[VERSION]/lib/node_modules/semo-plugin-z
```
...
:::warning
由于 `Semo` 的性能有些查，所以这个自动补全虽然能用，但是体验极差，不建议使用。
:::